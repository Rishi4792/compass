...
printf '# r\n\n## RECEIPT — build · t · PASS\n- [x] stage-end: %s\n' 'asked=yes · answer=continue' > "$T/bad/receipts.md"
bash "$SH" stage-end-gate "$T/good" >/dev/null 2>&1 || { echo "the ACCEPTING fixture was refused — the gate refuses everything"; exit 1; }
bash "$SH" stage-end-gate "$T/bad" >/dev/null 2>&1 && { echo "a stage end that does not say the cockpit printed was ACCEPTED — contract §7 requires it in every mode, no exception"; exit 1; }
exit 0

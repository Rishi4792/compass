...
ST_LOW="$SB/st-low.sh"; mkstub "$ST_LOW" "selftest: 405 passed, 0 failed" "$NAMES12"
ERR="$(COMPASS_RECON_SELFTEST_CMD="$ST_LOW" COMPASS_RECON_SMOKE_CMD="$SM_OK" bash "$RC" 2>&1 >/dev/null)"; RCC=$?
chk "$RCC" "1" "INV-RECON refuse: selftest 405 < floor 406"
chk "$(printf '%s' "$ERR" | grep -c 'refuse: floor-selftest')" "1" "INV-RECON reason code: floor-selftest"
SM_LOW="$SB/sm-low.sh"; mkstub "$SM_LOW" "──────── 221 passed, 0 failed ────────"
ERR="$(COMPASS_RECON_SELFTEST_CMD="$ST_OK" COMPASS_RECON_SMOKE_CMD="$SM_LOW" bash "$RC" 2>&1 >/dev/null)"
chk "$(printf '%s' "$ERR" | grep -c 'refuse: floor-smoke')" "1" "INV-RECON refuse+code: smoke 221 < floor 222"
ST_NONAME="$SB/st-noname.sh"; mkstub "$ST_NONAME" "selftest: 406 passed, 0 failed" "INV-ENGINEFIX INV-GRAMMAR"

...
printf '## 7. Steps\n- [x] **S1**\n- [ ] **S2**\n' > "$G/.claude/builds/midbuild/plan.md"
# ── v0.35 P2 — THIRTEEN ASSERTIONS CHANGED THEIR EXPECTED VALUE, AND WHY ─────────────────────────
# Every test below used to prove that a Human-gated build mid-step REFUSES a quiet turn-end. v0.35's
# contract forbids that: "Human-gated is the off switch, and INV-HUMAN-GATED-NEVER-REFUSES is what
# makes that true rather than merely claimed." So the expected value is now `{}`.
#
# What those tests really PROVED — that the guard identifies the right session and the right build,
# parses session_id from the field rather than the transcript path, falls back to the environment,
# trims a stored owner, and stays isolated across repositories — is not observable while the output
# is `{}` for every input. It is not abandoned: P6 adds the refusal on the AUTONOMOUS path with the
# same ownership rules, and each label below names the obligation to re-point it there. That list is
# also written into the build's PHASES.md, so it is an obligation on the phase rather than a comment.
printf '%s' "$(sg)" | grep -q '^{}$'; chk "$?" "0" "v0.35 mid-build (IN-PROGRESS · step 4/11) + owning session → {} — Human-gated is the off switch and never refuses"
# plan.md half-checked, NO IN-PROGRESS build receipt → still mid-build via (b)
printf '## RECEIPT — contract · midbuild · PASS\n- [x] x\n' > "$G/.claude/builds/midbuild/receipts.md"
printf '%s' "$(sg)" | grep -q '^{}$'; chk "$?" "0" "v0.35 plan.md half-checked + owning session → {} (Human-gated)"
...
printf '%s' "$(sg)" | grep -q '^{}$'; chk "$?" "0" "v0.35 same slug flipped to real mid-step + owner → still {} — the mode decides, not the step"
...
printf '%s' "$O" | grep -q '^{}$'; chk "$?" "0" "v0.35 S1 owning session sessX + mid-build → {} (Human-gated never refuses)"
...
O="$(ig '{"session_id":"sessX","stop_hook_active":false}')"; printf '%s' "$O" | grep -q '^{}$'; chk "$?" "0" "v0.35 S7 A(sessX)+B(sessY) both Human-gated: sessX stops → {} (P6 re-points this at the autonomous refusal, where naming the right build is observable again)"
O="$(ig '{"session_id":"sessY","stop_hook_active":false}')"; printf '%s' "$O" | grep -q '^{}$'; chk "$?" "0" "v0.35 S8 same: sessY stops → {} (P6 obligation, as S7)"
...
printf '%s' "$O" | grep -q '^{}$'; chk "$?" "0" "v0.35 S10b malformed progress.md → valid JSON and no crash (Human-gated → {})"
...
setA sessX
printf '%s' "$(ig '{"session_id":"sessX","stop_hook_active":false}')" | grep -q '^{}$'; chk "$?" "0" "v0.35 S12a first stop, step 3/9 → {} (Human-gated)"
...
printf '%s' "$(ig '{"session_id":"sessX","stop_hook_active":false}')" | grep -q '^{}$'; chk "$?" "0" "v0.35 S12c step advanced 3→4 → still {} (P6 re-points the re-arming test at the refusal it creates)"
...
printf '%s' "$O" | grep -q '^{}$'; chk "$?" "0" "v0.35 S15 different uuid in transcript_path + spaced JSON → {} without crashing (P6 re-asserts the session_id parse at the refusal)"
# S16 env fallback
setA sessX
( cd "$I" && printf '%s' '{"stop_hook_active":false}' | CLAUDE_CODE_SESSION_ID=sessX bash "$SH" stop-guard | grep -q '^{}$' ); chk "$?" "0" "v0.35 S16 no stdin session_id + env fallback → {} (P6 re-asserts the fallback at the refusal)"
setA sessX
...
printf '%s' "$(ig '{"session_id":"sessX","stop_hook_active":false}')" | grep -q '^{}$'; chk "$?" "0" "v0.35 S17 owner with trailing whitespace/newline → {} (P6 re-asserts the strict extract at the refusal)"
...
printf '%s' "$(cd "$RA" && printf '%s' '{"session_id":"sessX","stop_hook_active":false}' | bash "$SH" stop-guard)" | grep -q '^{}$'; chk "$?" "0" "v0.35 S14b converse: sessX in repoA → {} (P6 re-asserts cross-repo isolation at the refusal)"
...
# v0.35 P6 — RE-POINTED, and the meaning is preserved rather than the mechanism. What this assertion
# proved is that the autonomous path fires at EVERY continuable stage and not only at `build` — the
# v0.10 bug. P6 replaced the cross-session spawn with an in-turn REFUSAL, because driving the build
# from this turn and starting a second session are alternatives and doing both drives it twice. So
# the same fixture, at the same non-build stage, must now REFUSE. If the reorder is ever undone this
# still fails, which is what the assertion was for.
_sgo="$(sgz 2>/dev/null)"; sleep 0.1
chk "$(printf '%s' "$_sgo" | grep -c '"decision":"block"')" "1" "★ INV-STAGE: .auto-mode at a NON-build stage (plan) → the Stop hook REFUSES there (the v0.10 bug is fixed; v0.35 replaced the spawn with the refusal)"
chk "$(printf '%s' "$_sgo" | grep -c 'review-plan')" "1" "★ v0.35 INV-REASON-NAMES-THE-COMMAND: ...and the reason names the successor the walk worked out, not a stage typed into a string"
chk "$([ -f "$SG/SPAWNED" ] && echo spawned || echo none)" "none" "★ v0.35: ...and NO second session was spawned beside the refusal"

# ── v0.35 P6 — THE THIRTEEN OBLIGATIONS, DISCHARGED ──────────────────────────────────────────────
# P2 deleted the Human-gated refusal, and with it the only observable thirteen assertions had. Their
# expected value became `{}` and PHASES.md recorded, per row, what each had really proved. Those
# properties are observable again now that the AUTONOMOUS path refuses, so each is re-asserted here
# against the refusal P6 creates. This block is what "P6 is not complete until every row has a live
# assertion" means.
_p6r="$(mktemp -d)"
_p6mk() { # <repo> <slug> <owner-line> [progress-override]
  local rp="$1" sl="$2" ow="$3" pg="${4:-}"
  mkdir -p "$rp/.claude/builds/$sl" "$rp/.claude/builds/.locks"
  ( cd "$rp" && git init -q . >/dev/null 2>&1 ) || true
  printf '%s · fixture · status=plan · facets=library · touches=x\n' "$sl" >> "$rp/.claude/builds/INDEX"
  if [ -n "$pg" ]; then printf '%s' "$pg" > "$rp/.claude/builds/$sl/progress.md"
  else printf '# %s\n\n**Status:** plan\n**Stage:** plan\n**Next:** write the plan\n' "$sl" > "$rp/.claude/builds/$sl/progress.md"; fi
  printf '## RECEIPT — contract · %s · PASS\nok\n\n## RECEIPT — review-contract · %s · PASS\nok\n' "$sl" "$sl" > "$rp/.claude/builds/$sl/receipts.md"
...
# S1 — the reason names the owning build.
_p6mk "$_p6r/a" A "session=$_SIDA"
_o="$(_p6run "$_p6r/a" "{\"session_id\":\"$_SIDA\",\"stop_hook_active\":false}")"
chk "$(printf '%s' "$_o" | grep -c '"decision":"block"')" "1" "v0.35 P6/S1: an owned Autonomous build at a clean seam → the hook REFUSES"
chk "$(printf '%s' "$_o" | grep -c '\bA\b')" "1" "v0.35 P6/S1: ...and the reason NAMES that build"

# S7/S8 — two builds, two sessions: each session sees ITS OWN build named, not the neighbour's.
_p6mk "$_p6r/ab" A "session=$_SIDA"
_p6mk "$_p6r/ab" B "session=sessY"
_o="$(_p6run "$_p6r/ab" "{\"session_id\":\"$_SIDA\",\"stop_hook_active\":false}")"
chk "$([ "$(printf '%s' "$_o" | grep -c '\bA\b')" = "1" ] && [ "$(printf '%s' "$_o" | grep -c '\bB\b')" = "0" ] && echo 1 || echo 0)" "1" "v0.35 P6/S7: session A stops → the reason names A and not B"
_o="$(_p6run "$_p6r/ab" '{"session_id":"sessY","stop_hook_active":false}')"
chk "$([ "$(printf '%s' "$_o" | grep -c '\bB\b')" = "1" ] && [ "$(printf '%s' "$_o" | grep -c '\bA\b')" = "0" ] && echo 1 || echo 0)" "1" "v0.35 P6/S8: session Y stops → the reason names B and not A — and this only works because P5 made the walk reach row 2"

# S15 — session_id is read from the FIELD, never from a uuid inside transcript_path.
_p6mk "$_p6r/p" A "session=$_SIDA"
_o="$(_p6run "$_p6r/p" "{ \"transcript_path\" : \"/x/00000000-0000-0000-0000-000000000000.jsonl\" , \"session_id\" : \"$_SIDA\" , \"stop_hook_active\" : false }")"
chk "$(printf '%s' "$_o" | grep -c '"decision":"block"')" "1" "v0.35 P6/S15: a different uuid in transcript_path and spaced JSON → still refuses (the parse is keyed to the session_id FIELD)"

# S16 — the CLAUDE_CODE_SESSION_ID fallback when stdin carries no session_id.
_p6mk "$_p6r/e" A "session=$_SIDA"
_o="$(_p6run "$_p6r/e" '{"stop_hook_active":false}' "$_SIDA")"
chk "$(printf '%s' "$_o" | grep -c '"decision":"block"')" "1" "v0.35 P6/S16: no session_id on stdin, env fallback set → still refuses"

# S17 — a stored owner with trailing whitespace still matches.
_p6mk "$_p6r/w" A "session=$_SIDA  "
_o="$(_p6run "$_p6r/w" "{\"session_id\":\"$_SIDA\",\"stop_hook_active\":false}")"
chk "$(printf '%s' "$_o" | grep -c '"decision":"block"')" "1" "v0.35 P6/S17: an owner line with trailing whitespace still matches (strict extract + trim)"

# S14b — a session is quiet in another repository, but not globally invisible.
_p6mk "$_p6r/r1" A "session=$_SIDA"
_p6other="$(mktemp -d)"; ( cd "$_p6other" && git init -q . >/dev/null 2>&1 ) || true
_o="$(_p6run "$_p6other" "{\"session_id\":\"$_SIDA\",\"stop_hook_active\":false}")"
chk "$_o" "{}" "v0.35 P6/S14b: the same session stopping in a DIFFERENT repository → quiet"
_o="$(_p6run "$_p6r/r1" "{\"session_id\":\"$_SIDA\",\"stop_hook_active\":false}")"
chk "$(printf '%s' "$_o" | grep -c '"decision":"block"')" "1" "v0.35 P6/S14b: ...and in its OWN repository it still refuses — quiet is isolation, not global invisibility"

# S10b — a malformed progress.md must still yield valid JSON and never crash.
_p6mk "$_p6r/m" A "session=$_SIDA" "no status line at all, and no stage either"
_o="$(_p6run "$_p6r/m" "{\"session_id\":\"$_SIDA\",\"stop_hook_active\":false}")"
chk "$([ "${_o#\{}" != "$_o" ] && [ "${_o%\}}" != "$_o" ] && echo 1 || echo 0)" "1" "v0.35 P6/S10b: a malformed progress.md still yields a JSON object and does not crash the session"
...
_p6json_true="${_p6json_true/SID/$_SIDA}"
_o="$(_p6run "$_p6r/a" "$_p6json_true")"
chk "$_o" "{}" "v0.35 P6: stop_hook_active still short-circuits before anything else is read"
rm -rf "$_p6r" "${_p6other:-}"
...
ST_LOW="$SB/st-low.sh"; mkstub "$ST_LOW" "selftest: 555 passed, 0 failed" "$NAMES12"
ERR="$(COMPASS_RECON_SELFTEST_CMD="$ST_LOW" COMPASS_RECON_SMOKE_CMD="$SM_OK" bash "$RC" 2>&1 >/dev/null)"; RCC=$?
chk "$RCC" "1" "INV-RECON refuse: selftest 555 < floor 556"
chk "$(printf '%s' "$ERR" | grep -c 'refuse: floor-selftest')" "1" "INV-RECON reason code: floor-selftest"
SM_LOW="$SB/sm-low.sh"; mkstub "$SM_LOW" "──────── 550 passed, 0 failed ────────"
ERR="$(COMPASS_RECON_SELFTEST_CMD="$ST_OK" COMPASS_RECON_SMOKE_CMD="$SM_LOW" bash "$RC" 2>&1 >/dev/null)"
chk "$(printf '%s' "$ERR" | grep -c 'refuse: floor-smoke')" "1" "INV-RECON refuse+code: smoke 550 < floor 551"
ST_NONAME="$SB/st-noname.sh"; mkstub "$ST_NONAME" "selftest: 556 passed, 0 failed" "INV-ENGINEFIX INV-GRAMMAR"
...
echo "── v0.16.0 survive-the-cutover: canary/bake/burn-rate/watcher/abort gates ──"
FXR="$HERE/fixtures"
scv() { bash "$SH" "$@" >/dev/null 2>&1; echo $?; }   # run a subcommand, echo its exit code
# INV-ABORT — the mid-flight abort sentinel + behavioral loop-harness
ABT="$SB/abort-b"; mkdir -p "$ABT"; echo x > "$ABT/contract.md"
chk "$(scv abort-check "$ABT")" "0" "INV-ABORT: clear dir → 0"
bash "$SH" abort "$ABT" >/dev/null 2>&1; chk "$(scv abort-check "$ABT")" "3" "INV-ABORT: after abort → 3"
bash "$SH" abort "$ABT" >/dev/null 2>&1; chk "$(scv abort-check "$ABT")" "3" "INV-ABORT: idempotent double-set → 3"
bash "$SH" abort-clear "$ABT" >/dev/null 2>&1; chk "$(scv abort-check "$ABT")" "0" "INV-ABORT: after clear → 0"
bash "$SH" abort-clear "$ABT" >/dev/null 2>&1; chk "$(scv abort-check "$ABT")" "0" "INV-ABORT: idempotent double-clear → 0"
LH="$(bash "$FXR/abort/loop-harness.sh" "$SH" 2>&1)"; chk "$?" "0" "INV-ABORT: loop-harness exits 0 (halted)"
chk "$(printf '%s' "$LH" | grep -c 'HALTED-before-op-3 ran=2')" "1" "INV-ABORT: loop-harness halts BEFORE op 3 (ran=2, ops 3-5 never ran)"
# INV-BAKE — per-signal fail-closed, library mode, union-worst
chk "$(scv bake-gate "$FXR/bake/in-bound")" "0" "INV-BAKE: in-bound → 0"
chk "$(scv bake-gate "$FXR/bake/not-elapsed")" "1" "INV-BAKE: not-elapsed → non-zero"
chk "$(scv bake-gate "$FXR/bake/out-of-bound")" "1" "INV-BAKE: signal over ceiling → non-zero"
chk "$(scv bake-gate "$FXR/bake/no-bound")" "1" "INV-BAKE: window but no bound fail-closed → non-zero"
chk "$(scv bake-gate "$FXR/bake/no-reading-mem")" "1" "INV-BAKE: absent mem reading is NEVER in-bound → non-zero"
chk "$(scv bake-gate "$FXR/bake/partial-bound-no-mem")" "1" "INV-BAKE: missing mem ceiling fail-closed → non-zero"
chk "$(scv bake-gate "$FXR/bake/worst-of-multiline")" "1" "INV-BAKE: union takes WORST reading → non-zero"
chk "$(COMPASS_BAKE_OBSCMD=true bash "$SH" bake-gate "$FXR/bake/library-suites-green" >/dev/null 2>&1; echo $?)" "0" "INV-BAKE: LIBRARY bound re-runs suites green → 0"
chk "$(COMPASS_BAKE_OBSCMD=false bash "$SH" bake-gate "$FXR/bake/library-suites-green" >/dev/null 2>&1; echo $?)" "1" "INV-BAKE: LIBRARY bound suites RED → non-zero"
# INV-CANARY — independent green only; literal-none-only substitute
chk "$(scv canary-analysis "$FXR/canary/green")" "0" "INV-CANARY: independent green → 0"
chk "$(scv canary-analysis "$FXR/canary/substituted-bake")" "0" "INV-CANARY: none + bake-window → SUBSTITUTED-BAKE 0"
chk "$(scv canary-analysis "$FXR/canary/self-computed")" "1" "INV-CANARY: self-computed green (gold==slice) → non-zero"
chk "$(scv canary-analysis "$FXR/canary/gold-in-builddir")" "1" "INV-CANARY: gold reads own artifacts → non-zero"
chk "$(scv canary-analysis "$FXR/canary/substituted-no-window")" "1" "INV-CANARY: none without bake-window → non-zero"
chk "$(scv canary-analysis "$FXR/canary/synonym-na")" "1" "INV-CANARY: synonym (n/a) is NOT the substitute → non-zero"
chk "$(scv canary-analysis "$FXR/canary/missing-block")" "1" "INV-CANARY: segment but no block fail-closed → non-zero"
chk "$(scv canary-analysis "$FXR/canary/no-reconcile")" "1" "INV-CANARY: isolates reconcile-PASS clause (RB-M4) → non-zero"
chk "$(scv canary-analysis "$FXR/canary/no-route-smoke")" "1" "INV-CANARY: isolates route-smoke clause (RB-M4) → non-zero"
chk "$(scv canary-analysis "$FXR/canary/no-provenance")" "1" "INV-CANARY: isolates gold/slice provenance clause (RB-M4) → non-zero"
chk "$(scv canary-analysis "$FXR/canary/na")" "0" "INV-CANARY: no canary declared → N/A 0"
# INV-BURNRATE — a breach must auto-fire rollback and never promote
chk "$(scv canary-analysis "$FXR/canary/breach-no-rollback")" "1" "INV-BURNRATE: breach with no rollback-fired → non-zero"
chk "$(scv canary-analysis "$FXR/canary/breach-rollback")" "1" "INV-BURNRATE: breach + rollback never promotes → non-zero"
# INV-WATCHER — named or proven-armed only
chk "$(scv watcher-check "$FXR/watcher/named")" "0" "INV-WATCHER: named watcher + window → 0"
chk "$(scv watcher-check "$FXR/watcher/auto-rehearsed")" "0" "INV-WATCHER: --auto proven-armed rollback → 0"
chk "$(scv watcher-check "$FXR/watcher/placeholder-name")" "1" "INV-WATCHER: placeholder name rejected → non-zero"
chk "$(scv watcher-check "$FXR/watcher/auto-bare-armed")" "1" "INV-WATCHER: bare 'armed' rejected → non-zero"
chk "$(scv watcher-check "$FXR/watcher/neither")" "1" "INV-WATCHER: neither watcher nor armed → non-zero"
chk "$(scv watcher-check "$FXR/watcher/na")" "0" "INV-WATCHER: no cutover → N/A 0"
# INV-NA-EXPLICIT — the ship receipt must record all three; deploy:in-scope cannot fail-open
chk "$(scv ship-cutover-receipt-match "$FXR/cutover-receipt/complete")" "0" "INV-NA-EXPLICIT: all three recorded → 0"
chk "$(scv ship-cutover-receipt-match "$FXR/cutover-receipt/deploy-inscope-waived")" "0" "INV-NA-EXPLICIT: all-N/A + explicit waiver → 0"
chk "$(scv ship-cutover-receipt-match "$FXR/cutover-receipt/missing-bake")" "1" "INV-NA-EXPLICIT: missing bake line → non-zero"
chk "$(scv ship-cutover-receipt-match "$FXR/cutover-receipt/deploy-inscope-all-na")" "1" "INV-NA-EXPLICIT: deploy in-scope all-N/A fail-open guard → non-zero"
# INV-BC — byte-inert for a build that declares no cutover config
chk "$(scv bake-gate "$FXR/bake/na-not-declared")" "0" "INV-BC: bake-gate byte-inert (unconfigured) → 0"
chk "$(scv canary-analysis "$FXR/canary/na")" "0" "INV-BC: canary-analysis byte-inert (unconfigured) → 0"
chk "$(scv watcher-check "$FXR/watcher/na")" "0" "INV-BC: watcher-check byte-inert (unconfigured) → 0"
# review-build regression — the adversarial fan-out's attacks, each now fail-closed and pinned forever
chk "$(scv bake-gate "$FXR/bake/numeric-recon-prose")" "1" "RB-C1: numeric bound + 'reconciled' prose stays NUMERIC (mem enforced) → non-zero"
chk "$(scv bake-gate "$FXR/bake/stale-loose-first")" "1" "RB-C3: a loose bake-bound above the tight one does not win → non-zero"
chk "$(scv bake-gate "$FXR/bake/tab-window-no-obs")" "1" "RB-M5: a tab-indented bake-window is not read as absent → non-zero"
chk "$(scv bake-gate "$FXR/bake/fenced-observed")" "1" "RB-M6: a fenced bake-observed example is not a real reading → non-zero"
chk "$(scv canary-analysis "$FXR/canary/self-computed-ws")" "1" "RB-C2: gold≡slice modulo whitespace is self-computed → non-zero"
chk "$(scv watcher-check "$FXR/watcher/auto-placeholder-rollback")" "1" "RB-M1: a placeholder rollback command is not proof → non-zero"
chk "$(scv ship-cutover-receipt-match "$FXR/cutover-receipt/deploy-prose-outofscope")" "1" "RB-M2: 'out-of-scope' in deploy prose does not disable the fail-open guard → non-zero"
# review-build ROUND 2 regression — holes the first fixes left (re-attack of the fix diff)
chk "$(scv bake-gate "$FXR/bake/tilde-fenced-observed")" "1" "RB2-M1: a ~~~ tilde fence hides an example reading too → non-zero"
chk "$(scv canary-analysis "$FXR/canary/gold-in-builddir-upper")" "1" "RB2-M2: CONTRACT.MD (case-insensitive FS) is the build's own artifact → non-zero"
chk "$(scv ship-cutover-receipt-match "$FXR/cutover-receipt/deploy-inscope-na-noslash")" "1" "RB2-m3: NA without a slash still counts as N/A (fail-open guard fires) → non-zero"
# review-build ROUND 4 regression — a recorded FAILURE must not promote via an unanchored substring
chk "$(scv canary-analysis "$FXR/canary/reconcile-failed")" "1" "RB3-M2: a recorded '→ did NOT PASS' reconcile is not green → non-zero"
chk "$(scv canary-analysis "$FXR/canary/route-smoke-failed")" "1" "RB3-M1: a recorded '→ HTTP 500 (…200ms)' smoke is not green → non-zero"
# review-build ROUND 5 regression — take-WORST across lines/arrows, and the literal N/A-pass token
chk "$(scv canary-analysis "$FXR/canary/route-decoy")" "1" "RB4-M1: a failing route + a passing decoy route blocks (take-worst) → non-zero"
chk "$(scv watcher-check "$FXR/watcher/auto-multiexit")" "1" "RB4-M3: a recorded non-zero exit anywhere is not armed → non-zero"
chk "$(scv ship-cutover-receipt-match "$FXR/cutover-receipt/deploy-inscope-na-pass")" "1" "RB4-M2: the literal 'N/A-pass' token counts as N/A (fail-open guard fires) → non-zero"

echo "── v0.21.0 W1: schema-pin-gate (INV-SCHEMA-PIN) + perf-budget-gate (INV-PERFBUDGET) ──"
spg="$SB/v21-sp"
d="$spg/yes-ok"; mkdir -p "$d"; printf 'schema-touching: yes\nevolution-rules: additive-only\n\n| name | type | nullable |\n| id | int | no |\n' > "$d/contract.md"
bash "$SH" schema-pin-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-SCHEMA-PIN: schema-touching:yes + filled field-schema block → PASS"
d="$spg/yes-noblock"; mkdir -p "$d"; printf 'schema-touching: yes\n' > "$d/contract.md"
bash "$SH" schema-pin-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-SCHEMA-PIN: schema-touching:yes + no block + no N/A → FAIL"
d="$spg/yes-na"; mkdir -p "$d"; printf 'schema-touching: yes\nschema-pin: N/A — event stream, no field schema\n' > "$d/contract.md"
bash "$SH" schema-pin-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-SCHEMA-PIN: schema-touching:yes + explicit 'schema-pin: N/A — reason' → N/A-pass"
d="$spg/st-no"; mkdir -p "$d"; printf 'schema-touching: no\n' > "$d/contract.md"
bash "$SH" schema-pin-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-SCHEMA-PIN: schema-touching:no → N/A-pass"
d="$spg/legacy"; mkdir -p "$d"; printf '**Facets:** library\n' > "$d/contract.md"
bash "$SH" schema-pin-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-SCHEMA-PIN: legacy (schema-touching absent) → N/A-pass (INV-BC)"
d="$spg/nocontract"; mkdir -p "$d"
bash "$SH" schema-pin-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-SCHEMA-PIN: no contract.md → N/A-pass (guard-first INV-BC)"
pbg="$SB/v21-pb"
d="$pbg/decl"; mkdir -p "$d"; printf 'perf-budget: declared\np95 < 200ms · peak-mem 512MB · cost $0.01/req · SLO healthy-range 99.9%%\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-PERFBUDGET: declared p95+peak-mem+cost+SLO-range → PASS"
d="$pbg/na"; mkdir -p "$d"; printf 'perf-budget: N/A — plugin-sized, no runtime volume\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-PERFBUDGET: 'N/A — reason' → N/A-pass"
d="$pbg/nabare"; mkdir -p "$d"; printf 'perf-budget: N/A\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-PERFBUDGET: bare 'N/A' (no reason) → FAIL"
d="$pbg/nocost"; mkdir -p "$d"; printf 'perf-budget: declared\np95 < 200ms · peak-mem 512MB · SLO healthy-range 99.9%%\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-PERFBUDGET: declared missing cost → FAIL"
d="$pbg/noslo"; mkdir -p "$d"; printf 'perf-budget: declared\np95 < 200ms · peak-mem 512MB · cost $0.01/req\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-PERFBUDGET: declared missing SLO healthy-range → FAIL"
d="$pbg/absent"; mkdir -p "$d"; printf '**Facets:** library\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-PERFBUDGET: header absent (legacy) → N/A-pass (INV-BC)"
d="$pbg/nocontract"; mkdir -p "$d"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-PERFBUDGET: no contract.md → N/A-pass (guard-first)"
# INV-BC seam witness: a receipts-only dir (no contract.md) passes the contract seam byte-inert (the cc-shape)
scw="$SB/v21-seam/nc"; mkdir -p "$scw"; printf '## RECEIPT — contract · nc · PASS\n- [x] ok\n' > "$scw/receipts.md"
bash "$SH" gate "$scw" contract >/dev/null 2>&1; chk "$?" "0" "INV-BC: contract seam N/A-passes a receipts-only dir (no contract.md) — INV-SCHEMA-PIN + INV-PERFBUDGET guard-first"

echo "── v0.21.0 W3: expand-contract / backfill-recon / rollback-fwdcompat gates ──"
ecg="$SB/v21-ec"
d="$ecg/ok"; mkdir -p "$d"; printf 'schema-touching: yes\nmigration-phase: expand\nold-code-probe: v0 read vs v1 schema\ndry-run: prod-shaped copy\n' > "$d/contract.md"
bash "$SH" expand-contract-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-EXPAND-CONTRACT: expand + old-code-probe + prod-shaped dry-run → PASS"
d="$ecg/noprobe"; mkdir -p "$d"; printf 'schema-touching: yes\nmigration-phase: expand\ndry-run: prod-shaped\n' > "$d/contract.md"
bash "$SH" expand-contract-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-EXPAND-CONTRACT: missing old-code-probe → FAIL"
d="$ecg/nodefer"; mkdir -p "$d"; printf 'schema-touching: yes\nmigration-phase: contract\nold-code-probe: x\ndry-run: prod-shaped\n' > "$d/contract.md"
bash "$SH" expand-contract-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-EXPAND-CONTRACT: migration-phase:contract not deferred → FAIL"
d="$ecg/stno"; mkdir -p "$d"; printf 'schema-touching: no\n' > "$d/contract.md"
bash "$SH" expand-contract-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-EXPAND-CONTRACT: schema-touching:no → N/A-pass"
d="$ecg/nc"; mkdir -p "$d"
bash "$SH" expand-contract-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-EXPAND-CONTRACT: no contract.md → N/A-pass (guard-first INV-BC)"
bfg="$SB/v21-bf"
d="$bfg/ok"; mkdir -p "$d"; printf 'backfill: yes\nbackfill-recon: count + checksum tie to source\n' > "$d/contract.md"
bash "$SH" backfill-recon-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-BACKFILL-RECON: backfill:yes + count+checksum record → PASS"
d="$bfg/norec"; mkdir -p "$d"; printf 'backfill: yes\n' > "$d/contract.md"
bash "$SH" backfill-recon-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-BACKFILL-RECON: backfill:yes + no record → FAIL"
d="$bfg/destr"; mkdir -p "$d"; printf 'destructive-backfill: yes\n' > "$d/contract.md"
bash "$SH" backfill-recon-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-BACKFILL-RECON: destructive-backfill:yes + no record → FAIL"
d="$bfg/no"; mkdir -p "$d"; printf 'backfill: no\n' > "$d/contract.md"
bash "$SH" backfill-recon-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-BACKFILL-RECON: backfill:no → N/A-pass"
d="$bfg/absent"; mkdir -p "$d"; printf '**Facets:** library\n' > "$d/contract.md"
bash "$SH" backfill-recon-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-BACKFILL-RECON: absent (legacy) → N/A-pass"
d="$bfg/nc"; mkdir -p "$d"
bash "$SH" backfill-recon-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-BACKFILL-RECON: no contract.md → N/A-pass (guard-first)"
rbg="$SB/v21-rb"
d="$rbg/ok"; mkdir -p "$d"; printf 'schema-touching: yes\nrollback data-safety: old-code reads new-version writes → OK\n' > "$d/contract.md"
bash "$SH" rollback-fwdcompat-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-ROLLBACK-FWDCOMPAT: schema change + recorded safety line → PASS"
d="$rbg/miss"; mkdir -p "$d"; printf 'schema-touching: yes\n' > "$d/contract.md"
bash "$SH" rollback-fwdcompat-gate "$d" >/dev/null 2>&1; chk "$?" "1" "INV-ROLLBACK-FWDCOMPAT: schema change + missing record → FAIL"
d="$rbg/no"; mkdir -p "$d"; printf 'schema-touching: no\n' > "$d/contract.md"
bash "$SH" rollback-fwdcompat-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-ROLLBACK-FWDCOMPAT: no schema/data change → N/A-pass"
d="$rbg/nc"; mkdir -p "$d"
bash "$SH" rollback-fwdcompat-gate "$d" >/dev/null 2>&1; chk "$?" "0" "INV-ROLLBACK-FWDCOMPAT: no contract.md → N/A-pass (guard-first)"
...
d="$spr/gc-failure"; mkdir -p "$d"; printf 'ci: yes\ngreen-ci: rerun after the earlier failure is now green\n' > "$d/contract.md"
bash "$SH" green-ci-gate "$d" >/dev/null 2>&1; chk "$?" "0" "RB-R3: green-ci 'earlier failure is now green' (green rerun; the noun 'failure' is not a NOT-green phrase) → PASS (a lying-but-green-worded record is the review re-challenge's job)"
d="$spr/gc-succeeded"; mkdir -p "$d"; printf 'ci: yes\ngreen-ci: merge succeeded, zero failures\n' > "$d/contract.md"
bash "$SH" green-ci-gate "$d" >/dev/null 2>&1; chk "$?" "0" "RB-R3-M2: green-ci 'merge succeeded, zero failures' (succeeded=positive; 'failures' not a NOT-green phrase) → PASS (no false-reject)"
d="$spr/pb-descriptor"; mkdir -p "$d"; printf 'perf-budget: declared\np95 latency: 200ms · peak memory under load: 512MB · cost $0.02/req · SLO healthy-range 99.9%%\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "RB-R3-M1: perf-budget with a descriptor ('p95 latency: 200ms') → PASS (decoupled name+literal; no false-reject)"
d="$spr/pb-99th"; mkdir -p "$d"; printf 'perf-budget: declared\np95 (99th): 1.2s · peak-mem 512MB · cost $0.02 · SLO 99.9%%\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "RB-R3: perf-budget 'p95 (99th): 1.2s' (percentile descriptor with its own digits) → PASS (decoupled design, no proximity mis-parse)"
d="$spr/pb-spelled"; mkdir -p "$d"; printf 'perf-budget: declared\np95 latency 200ms, peak memory 2 GB, cost 3 cents/req, SLO healthy-range 99.5%%\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "RB-R3: perf-budget with spelled-out cost ('3 cents/req') + memory ('2 GB') → PASS"
d="$spr/pb-gib"; mkdir -p "$d"; printf 'perf-budget: declared\np95 200ms, peak-mem 2 GiB, cost $0.02, SLO 99.9%%\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "RB-R4-F1: perf-budget with a BINARY memory unit ('2 GiB', k8s/JVM) → PASS (no false-reject)"
d="$spr/pb-multiline"; mkdir -p "$d"; printf 'perf-budget:\n- p95 200ms\n- peak-mem 512MB\n- cost $0.02/req\n- SLO 99.9%%\n' > "$d/contract.md"
bash "$SH" perf-budget-gate "$d" >/dev/null 2>&1; chk "$?" "0" "RB-R4-F2: multi-line perf-budget block (bare header + budget below) with all axes+literals → PASS (enforced, not silently N/A)"
d="$spr/pb-mlbroken"; mkdir -p "$d"; printf 'perf-budget:\nfast enough, cheap enough\n' > "$d/contract.md"
...
# ── v0.28.0 "always clarity" — renderer edge states + the two mid-build bug fixes ──
V28="$SB/v28"; mkdir -p "$V28"
mk() { # <name> <n-steps> <n-checked> [mode]
  local d="$V28/$1" n="$2" c="$3" m="${4:-Human-gated}" i
  mkdir -p "$d"; : > "$d/plan.md"; : > "$d/receipts.md"
  for i in $(seq 1 "$n"); do
    if [ "$i" -le "$c" ]; then printf -- '- [x] **%s · s%s.**\n' "$i" "$i" >> "$d/plan.md"
      printf '## RECEIPT — build · %s · IN-PROGRESS · step %s/%s\n- [x] ok\n\n' "$1" "$i" "$n" >> "$d/receipts.md"
    else printf -- '- [ ] **%s · s%s.**\n' "$i" "$i" >> "$d/plan.md"; fi
  done
  printf '# Progress\nstatus: build\nmode: %s\n**Next:** keep going\n' "$m" > "$d/progress.md"
}
# 0 steps · 1 step · exactly 12 (full list) · exactly 13 (windowing kicks in) · k=0 · k=n
mkdir -p "$V28/zero"; : > "$V28/zero/plan.md"; : > "$V28/zero/receipts.md"; printf '# Progress\nmode: Human-gated\n' > "$V28/zero/progress.md"
( bash "$SH" progress-card "$V28/zero" >/dev/null 2>&1 ); chk "$?" "0" "v0.28 INV-CARD: a plan with 0 steps renders cleanly instead of crashing"
mk one 1 0;   ( bash "$SH" progress-card "$V28/one" >/dev/null 2>&1 );   chk "$?" "0" "v0.28 INV-CARD: a 1-step plan renders"
mk twelve 12 0; chk "$(bash "$SH" progress-card "$V28/twelve" | grep -cE '^  [✓▶!·] [0-9]+ ')" "12" "v0.28 INV-CARD-CAP: exactly 12 steps renders the FULL list (boundary, not windowed)"
mk thirteen 13 0; chk "$([ "$(bash "$SH" progress-card "$V28/thirteen" | grep -cE '^  [✓▶!·] [0-9]+ ')" -lt 13 ] && echo 1 || echo 0)" "1" "v0.28 INV-CARD-CAP: 13 steps windows (the other side of the boundary)"
mk kzero 5 0; chk "$(bash "$SH" progress-card "$V28/kzero" | grep -c '0 done')" "1" "v0.28 INV-CARD: k=0 (nothing done yet) tallies correctly"
mk kall 5 5;  chk "$(bash "$SH" progress-card "$V28/kall" | grep -c '5 done')" "1" "v0.28 INV-CARD: k=n (everything done) tallies correctly"
# missing files degrade honestly rather than crashing
mkdir -p "$V28/noplan"; printf '# Progress\nmode: Human-gated\n' > "$V28/noplan/progress.md"
( bash "$SH" progress-card "$V28/noplan" >/dev/null 2>&1 ); chk "$?" "0" "v0.28 INV-CARD: a missing plan.md degrades to an honest short block, exit 0"
mk noprog 3 0; rm -f "$V28/noprog/progress.md"
( bash "$SH" orient --where "$V28/noprog" >/dev/null 2>&1 ); chk "$?" "0" "v0.28 INV-ORIENT: a missing progress.md does not crash the MID block"
chk "$(bash "$SH" orient --where "$V28/noprog" | grep -c 'mode: not set')" "1" "v0.28 INV-MODE-VISIBLE: an absent mode reads 'not set' — never a guessed value"
# kill-switch: suppress printing, never recording
mk quiet 4 1
chk "$(COMPASS_QUIET=1 bash "$SH" progress-card "$V28/quiet" | wc -c | tr -d ' ')" "0" "v0.28 kill-switch: COMPASS_QUIET silences the card"
chk "$(COMPASS_QUIET=1 bash "$SH" orient --new | wc -c | tr -d ' ')" "0" "v0.28 kill-switch: COMPASS_QUIET silences the orientation block"
# INV-LOCALE-SAFE — last_block's multibyte bracket-range bug, both directions
chk "$(LC_ALL=C bash -c 'source "'"$SH"'" 2>/dev/null; last_block "'"$V28"'/quiet/receipts.md" build | wc -l | tr -d " "')" "$(LC_ALL=en_US.UTF-8 bash -c 'source "'"$SH"'" 2>/dev/null; last_block "'"$V28"'/quiet/receipts.md" build | wc -l | tr -d " "' 2>/dev/null)" "v0.28 INV-LOCALE-SAFE: last_block returns the same receipt under LC_ALL=C and a UTF-8 locale"
# INV-TERMINAL-STATUS — lowercase statuses are what ship actually writes
chk "$(bash -c 'source "'"$SH"'" 2>/dev/null; is_terminal shipped && echo 0 || echo 1')" "0" "v0.28 INV-TERMINAL-STATUS: lowercase 'shipped' is terminal"
chk "$(bash -c 'source "'"$SH"'" 2>/dev/null; is_terminal rolled-back && echo 0 || echo 1')" "0" "v0.28 INV-TERMINAL-STATUS: lowercase 'rolled-back' is terminal"
chk "$(bash -c 'source "'"$SH"'" 2>/dev/null; is_terminal build && echo 0 || echo 1')" "1" "v0.28 INV-TERMINAL-STATUS: a mid-lifecycle status is NOT terminal"

...
## Procedure
1. **Read `.claude/builds/CURRENT`** to get the active slug. If it's missing, list `.claude/builds/*/` and ask the user which build to resume (don't guess).
2. Read that build's `progress.md` (the cursor) and, if building, `plan.md` (its **checkboxes are the authoritative** progress — if `progress.md` and the checkboxes disagree, trust the checkboxes). Read `contract.md` (the invariant) and `review-ledger.md` (open issues).

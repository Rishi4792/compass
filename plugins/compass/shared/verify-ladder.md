...
- **Two kinds of UI check — use both, don't substitute one for the other:**
  ...
  - *Design-intent fidelity* (layout, hierarchy, spacing, typography, color, every state) → the **BRUTAL, NON-NEGOTIABLE design gate** (v0.5.0): render the built UI vs the mockup at every viewport + state, log each difference as an open row in `design-ledger.md`, and **loop until zero open rows** — one drift = FAIL. The bar is **identical** for an HTML mockup (add `compass.sh design-style-diff` token checks — necessary, not sufficient) or a flat image (disciplined element-by-element side-by-side reading). `compass.sh converge-gate` blocks convergence until both the correctness AND design ledgers are clean. (No captured design intent for a web build = contract gap → FAIL.)
- **Non-ceremonial verify (v0.5.0):** review-build **does not re-run** the build's own checks — it independently renders the live product on **real data** and adversarially reads the actual values + pixels. Every check must be **falsifiable** (able to fail if broken); a tautological or screenshot-only "looks right" check is deleted, not counted.

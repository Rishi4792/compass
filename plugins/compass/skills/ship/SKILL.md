...
6. **Secret-scan the release patch (v0.15.0)** — once the release commit exists, `compass.sh secret-scan --commits <prev-tag>..HEAD` → 0 hits, so the actual committed patch is proven clean (review-build scans the pre-commit working tree; this covers anything the release commit itself introduces). **Any hit → STOP, scrub, re-commit before push.**
   - **Screenshot secret hygiene (v0.21.0, INV-IMG-SECRET):** any ship/launch screenshot shows only redacted / `test-tenant` data — never screenshot a real-secret view. The release-patch `secret-scan is text-only` (no image/OCR scanner — the plugin is dependency-free), so a screenshot's pixels are never scanned; this is a disclosure + reviewer checklist, not an image scan.
...
**Program-ledger advance (v0.22.0) — on EITHER SHIPPED path above, immediately AFTER the SHIPPED write:** run this ONLY if the build is a program member — `hdr_get .claude/builds/<slug>/contract.md program:` non-empty (a standalone build with no `program:` header skips it entirely — INV-BC, byte-inert). Then, IN ORDER: (1) create the release tag **idempotently** — `git rev-parse -q --verify "v<version>^{commit}" >/dev/null || git tag v<version>` (ship OWNS this ONE local tag; `RELEASING.md` pushes it at the human gate — **never re-tag**); (2) `compass.sh program-advance <program-name> <slug> v<version>` — the guard requires a REAL tag bound to THIS build, so **the tag MUST precede the advance** (in `--auto` the push is deferred to the human gate, so the LOCAL tag still resolves for the guard). Record a `program-advance: <program · slug → shipped v<version>>` receipt line.

**Operability ledger (v0.23.0) — immediately AFTER the SHIPPED write, on EITHER path:** `( compass.sh dora-record .claude/builds/<slug> SHIPPED ) >/dev/null 2>&1 || true` — a SUBSHELL + `>/dev/null` so an internal `die` (e.g. a stale `dora` lock) can NEVER abort the terminal write and no `ok` line leaks (additive: it appends a gitignored `DORA.md` row, changing no existing output). On a **rollback** (§7) record `ROLLED-BACK` the same way. Record a `dora: <slug> outcome=SHIPPED …` receipt line.

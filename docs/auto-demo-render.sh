#!/usr/bin/env bash
# auto-demo-render.sh — render docs/auto-demo.html (5 designed scenes) into docs/auto-demo.gif.
...
ff -i "$TMP/full.mp4" -i "$TMP/pal.png" -lavfi "fps=18,scale=1040:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=3" "$HERE/auto-demo.gif"
echo "rendered: $HERE/auto-demo.gif"
